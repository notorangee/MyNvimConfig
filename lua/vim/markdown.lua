...
function! GetVisualSelection()
    let l:saved_reg_a = getreg('a')
    let l:saved_reg_a_type = getregtype('a')
    normal! gv"ay
    let l:selection = getreg('a')
    call setreg('a', l:saved_reg_a, l:saved_reg_a_type)
    return l:selection
endfunction
...
function ReviseMarkdown( selectModeType, selectVisualMode, selectTitle )
  if &filetype == 'markdown'
    ...
    let g:markdownContent = a:selectVisualMode ?  GetVisualSelection() : expand("<cword>")
    let g:insertType = ""
...
autocmd Filetype markdown inoremap ,j <br><Enter>
...
nnoremap <silent> ,j :call setline(".", getline(".")."<br>")<CR>
